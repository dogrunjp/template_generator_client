      ...
      const visit = (errors, history, current, node, schemaPath, trace = {}, { constProp } = {}) => {
        ...
        const evaluateDeltaDynamic = (delta) => {
          ...
          if (dyn.props && (delta.properties || []).includes(true) && !stat2.properties.includes(true)) {
            ...
            for (const sym of delta.propertiesVars || [])
              fun.write("%s[0].push(%s)", dyn.props, sym);
          }
        };
        ...
        const lintRequired = (properties, patterns) => {
          ...
          for (const key of stat2.required)
            enforce(known(key), `Unknown required property:`, key);
        };
        const finalLint = [];
        const checkNumbers = () => {
        ...
        const checkObjects = () => {
          ...
          for (const dependencies of ["dependencies", "dependentRequired", "dependentSchemas"]) {
            if (dependencies !== "dependencies" && getMeta().dependentUnsupported)
              continue;
        ...
        const checkGeneric = () => {
          ...
          if (thenOrElse || allowUnusedKeywords)
            handle("if", ["object", "boolean"], (ifS) => {
              ...
              if (!handleThen && !deltaEmpty(deltaIf))
                handleThen = () => evaluateDeltaDynamic(deltaIf);
          ...
          prevWrap(stat2.patterns.length > 0 || stat2.dyn.patterns.length > 0 || stat2.unknown, () => {
            if (stat2.properties.includes(true)) {
              ...
              if (isDynamic(stat2).properties) {
                ...
                if (node.unevaluatedProperties === false)
                  lintRequired(stat2.properties, stat2.patterns);
    ...
    var validator = (schema, { parse = false, multi = false, jsonCheck = false, isJSON = false, schemas = [], ...opts } = {}) => {
      ...
      if (multi) {
        ...
        for (const ref of refs.slice(0, -1))
          fun.write("%s,", ref);
        if (refs.length > 0)
          fun.write("%s", refs[refs.length - 1]);
        fun.write("]");
        if (parse)
          fun.write(".map(parseWrap)");
        else if (jsonCheck)
          fun.write(".map(jsonCheckWrap)");
      } else {
        if (parse)
